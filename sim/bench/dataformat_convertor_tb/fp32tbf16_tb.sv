...
import "DPI-C" function void FromFloatBits_bf(
...
initial begin
  ...
  for (i = 'h00000000; i <= 'hffffffff; i = i + 1) begin
    float32_in = i;
    in_data = i;
    FromFloatBits_bf(float32_in, float16_out_ref, 0);
    #10;
    ...
    if (float16_out !== float16_out_ref && (!u_dut.nan)) begin
      ...
      $display("carry_p: %h", u_dut.carry_p);
      $display("g: %h", u_dut.g);
      $display("r: %h", u_dut.r);
      $display("s: %h", u_dut.s);
      $display("inf: %h", u_dut.inf);
      $display("nan: %h", u_dut.nan);
      $display("zero: %h", u_dut.zero);
      // $display("bits_rounded: %h", u_dut.bits_rounded);
      // $display("golden:");
      // FromFloatBits_bf(float32_in, float16_out_ref, 1);
      $finish;
    end
  end
