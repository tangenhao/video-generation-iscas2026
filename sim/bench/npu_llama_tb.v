...
initial begin
  $fsdbDumpfile("npu_llama_tb.fsdb");
  $fsdbDumpvars(0);
  // $fsdbDumpMDA();
end
...
parameter INSN_ADDR                      = 'h0; 

// Llama Block Specific DDR Addresses (divided by 0x20 from C defines in addr_for_llama.h)
parameter BLOCK_INPUT_ADDR               = 'h010000; // C_ADDR: 0x0200000
parameter BLOCK_OUTPUT_ADDR              = 'h018000; // C_ADDR: 0x0300000
parameter ATTN_NORM_GAMMA_ADDR           = 'h020000; // C_ADDR: 0x0400000
parameter ATTN_QUERY_WEIGHT_ADDR         = 'h028000; // C_ADDR: 0x0500000
parameter ATTN_KEY_WEIGHT_ADDR           = 'h030000; // C_ADDR: 0x0600000
parameter ATTN_VALUE_WEIGHT_ADDR         = 'h038000; // C_ADDR: 0x0700000
parameter ATTN_OUTPUT_WEIGHT_ADDR        = 'h040000; // C_ADDR: 0x0800000 
parameter FFN_NORM_GAMMA_ADDR            = 'h048000; // C_ADDR: 0x0900000
parameter MLP_GATE_WEIGHT_ADDR           = 'h050000; // C_ADDR: 0x0A00000
parameter MLP_UP_WEIGHT_ADDR             = 'h058000; // C_ADDR: 0x0B00000
parameter MLP_DOWN_WEIGHT_ADDR           = 'h060000; // C_ADDR: 0x0C00000

// Temporary Buffers
parameter ATTN_NORM_OUTPUT_ADDR          = 'h068000 ; // C_ADDR: 0x0D00000  
parameter ATTN_NORM_OUTPUT_HALF_ADDR     = 'h070000; //  C_ADDR: 0x0E00000  
parameter ATTN_QUERY_TEMP_ADDR           = 'h078000; //  C_ADDR: 0x0F00000
parameter ATTN_KEY_TEMP_ADDR             = 'h080000; //  C_ADDR: 0x1000000
parameter ATTN_VALUE_TEMP_ADDR           = 'h088000; //  C_ADDR: 0x1100000
parameter ATTN_SCORE_TEMP_ADDR           = 'h090000; //  C_ADDR: 0x1200000
parameter ATTN_PROBE_TEMP_ADDR           = 'h098000; //  C_ADDR: 0x1300000
parameter ATTN_OUTPUT_TEMP_ADDR          = 'h0A0000; //  C_ADDR: 0x1400000
parameter ATTN_OUTPUT_PROJ_ADDR          = 'h0A8000; //  C_ADDR: 0x1500000  // Final projected output of Attention
parameter RESIDUAL_AFTER_ATTN_ADDR       = 'h0B0000; //  C_ADDR: 0x1600000  // input_hs + attn_out (fp32)
parameter FFN_NORM_OUTPUT_ADDR           = 'h0B8000; //  C_ADDR: 0x1700000  // Output of 2nd RMSNorm (fp32)
parameter FFN_NORM_OUTPUT_HALF_ADDR      = 'h0C0000; //  C_ADDR: 0x1800000  // Output of 2nd RMSNorm (fp16 after conversion for MLP)
parameter MLP_GATE_OUTPUT_ADDR           = 'h0C8000; //  C_ADDR: 0x1900000
parameter MLP_UP_OUTPUT_ADDR             = 'h0D0000; //  C_ADDR: 0x1A00000
parameter MLP_MUL_OUTPUT_ADDR            = 'h0D8000; // C_ADDR: 0x1B00000
parameter MLP_FINAL_PROJ_OUTPUT_ADDR     = 'h0E0000; // C_ADDR: 0x1C00000  // MLP raw output

// VCU Code and LUTs
parameter LLAMA_BLOCK_VCUCODE_ADDR       = 'h0E8000; // C_ADDR: 0x1D00000
parameter LLAMA_BLOCK_REC_LUT_ADDR       = 'h0F0000; // C_ADDR: 0x1E00000
parameter LLAMA_BLOCK_LOG_LUT_ADDR       = 'h0F0800; // C_ADDR: 0x1E10000
parameter LLAMA_BLOCK_EXP_LUT_ADDR       = 'h0F1000; // C_ADDR: 0x1E20000
parameter LLAMA_BLOCK_RSQRT_LUT_ADDR     = 'h0F1800; // C_ADDR: 0x1E30000
parameter LLAMA_BLOCK_TANH_LUT_ADDR      = 'h0F2000; // C_ADDR: 0x1E40000
parameter LLAMA_BLOCK_SIGMOID_LUT_ADDR   = 'h0F2800; // C_ADDR: 0x1E50000
parameter LLAMA_BLOCK_MISH_LUT_ADDR      = 'h0F3000; // C_ADDR: 0x1E60000
parameter LLAMA_BLOCK_SWISH_LUT_ADDR     = 'h0F3800; // C_ADDR: 0x1E70000
parameter LLAMA_BLOCK_GELU_LUT_ADDR      = 'h0F4000; // C_ADDR: 0x1E80000
parameter LLAMA_BLOCK_SINCOS_LUT_ADDR    = 'h0F4800; // C_ADDR: 0x1E90000

// Other Parameters
parameter ATTN_FREQ_CLS_ADDR                  = 'h100000 ; // C_ADDR: 0x2000000
parameter ATTN_MASK_ADDR                      = 'h108000 ; // C_ADDR: 0x2100000

initial begin
  $readmemh("../memory_llama/insn.txt", u_full_slave_ddr0.data_mem, INSN_ADDR);

  $readmemh("../memory_llama/block_input_hidden_state.txt", u_full_slave_ddr0.data_mem, BLOCK_INPUT_ADDR);
  $readmemh("../memory_llama/block_attn_norm_gamma.txt", u_full_slave_ddr0.data_mem, ATTN_NORM_GAMMA_ADDR);
  $readmemh("../memory_llama/block_attn_query_weight.txt", u_full_slave_ddr0.data_mem, ATTN_QUERY_WEIGHT_ADDR);
  $readmemh("../memory_llama/block_attn_key_weight.txt", u_full_slave_ddr0.data_mem, ATTN_KEY_WEIGHT_ADDR);
  $readmemh("../memory_llama/block_attn_value_weight.txt", u_full_slave_ddr0.data_mem, ATTN_VALUE_WEIGHT_ADDR);
  $readmemh("../memory_llama/block_attn_output_weight.txt", u_full_slave_ddr0.data_mem, ATTN_OUTPUT_WEIGHT_ADDR);
  $readmemh("../memory_llama/block_ffn_norm_gamma.txt", u_full_slave_ddr0.data_mem, FFN_NORM_GAMMA_ADDR);
  $readmemh("../memory_llama/block_mlp_gate_weight.txt", u_full_slave_ddr0.data_mem, MLP_GATE_WEIGHT_ADDR);
  $readmemh("../memory_llama/block_mlp_up_weight.txt", u_full_slave_ddr0.data_mem, MLP_UP_WEIGHT_ADDR);
  $readmemh("../memory_llama/block_mlp_down_weight.txt", u_full_slave_ddr0.data_mem, MLP_DOWN_WEIGHT_ADDR);

  $readmemh("../memory_llama/vcucode.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_VCUCODE_ADDR);

  $readmemh("../memory/reciprocal.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_REC_LUT_ADDR);
  $readmemh("../memory/log.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_LOG_LUT_ADDR);
  $readmemh("../memory/exp.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_EXP_LUT_ADDR);
  $readmemh("../memory/rsqrt.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_RSQRT_LUT_ADDR);
  $readmemh("../memory/tanh.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_TANH_LUT_ADDR);
  $readmemh("../memory/sigmoid.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_SIGMOID_LUT_ADDR);
  $readmemh("../memory/mish.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_MISH_LUT_ADDR);
  $readmemh("../memory/swish.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_SWISH_LUT_ADDR);
  $readmemh("../memory/gelu.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_GELU_LUT_ADDR);
  $readmemh("../memory/sincos.txt", u_full_slave_ddr0.data_mem, LLAMA_BLOCK_SINCOS_LUT_ADDR);

  $readmemh("../memory_llama/freq_cls_concat.txt", u_full_slave_ddr0.data_mem, ATTN_FREQ_CLS_ADDR);
  $readmemh("../memory_llama/mask_concat.txt", u_full_slave_ddr0.data_mem, ATTN_MASK_ADDR);
end

parameter psum_bits = 16;
parameter conv_len  = 2048;

reg [AXI4_FULL_M_AXI_RDATA_WIDTH-1:0] gt_reg [0:conv_len];
initial begin
  $readmemh("../memory_llama/block_output_hidden_state_ref.txt", gt_reg);
end
...
task check(integer address);
  begin
    // 打开结果输出文件
    integer result_file;
    result_file = $fopen("../memory_llama/result.txt", "w");
    ...
    for (i=0;i<=(conv_len);i=i+1) begin
      ...
      // 如果golden或结果为x,则不计入错误统计
      if (gt_reg[i] === 'hx || u_full_slave_ddr0.data_mem[address + i] === 'hx) begin
        // 跳过不定态的比较
      end
      else if (gt_reg[i]==u_full_slave_ddr0.data_mem[address + i]) begin
        cor_cnt =  cor_cnt + 1;
      end
      else begin
        err_cnt = err_cnt + 1;
