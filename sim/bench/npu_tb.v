...
initial begin
 // rst; 
  ...
  @(negedge u_apb_master.pready) #10cmd_in_wr(cmd, 320, 2);
  cmd_vld = 1;
  @(negedge apb4_pclk) cmd_vld = 0;
...
initial begin
  ...
  $readmemh("../memory/config.txt", u_full_slave_ddr.data_mem, 'hd0000);
end
...
always @(posedge logic_clk) begin
  if (u_npu_top_chiplet.u_npu_top.global_done) begin
    for (i=0;i<=(conv_len);i=i+1) begin
      $fdisplay(re_file, "%h", u_full_slave_ddr.data_mem['h30000+i]);
        if (gt_reg[i]==u_full_slave_ddr.data_mem['h30000+i]) begin
          cor_cnt =  cor_cnt + 1;
          ...
          $display("re is %h", u_full_slave_ddr.data_mem['h30000+i]);    
          $display("mismatch number is %d\n", err_cnt);
        end
      end
