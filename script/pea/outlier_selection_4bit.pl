...
print "==== INFO: Generating outlier_selection_4bit ==== \n";
print "lane: ", $lane, "\n";
print "p: ", $p, "\n";
...
print $fh "output wire [3:0] outlier;\n";
print $fh "\n";
...
print $fh "\n";
for (my $i = 0; $i < ($index_width/8); $i++) {
  print $fh "wire [31:0] data_$i;\n";
}
print $fh "\n";
for (my $i = 0; $i < ($index_width/8); $i++) {
  print $fh "assign data_$i = data[", 32*$i+31, ":", 32*$i, "];\n";
}
print $fh "\n";
for (my $i = 0; $i < ($index_width/8); $i++) {
  print $fh "reg [3:0] outlier_$i;\n";
}
print $fh "\n";
for (my $i = 0; $i < ($index_width/8); $i++) {
  ...
  for (my $j = 0; $j < 8; $j++) {
    print $fh "      8'b", '0' x $j, '1', 'x' x (7-$j), ": outlier_", $i, " = data_", $i, "[", 4*(7-$j)+3, ":", 4*(7-$j), "];\n";
  }
  print $fh "      default: outlier_", $i, " = 4'b0;\n";
  print $fh "    endcase\n";
  print $fh "  end\n";
  print $fh "  else begin\n";
  print $fh "    outlier_", $i, " = 4'b0;\n";
  print $fh "  end\n";
  print $fh "end\n";
  print $fh "\n";
}
