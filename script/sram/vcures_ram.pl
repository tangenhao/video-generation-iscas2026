...
print "==== INFO: Generating vcures_ram ==== \n";
print "width: ", $width, "\n";
print "depth: ", $depth, "\n";
print "bank: ", $bank, "\n";
...
print $fh "module vcures_ram(\n";
print $fh "  clk, rst_n,\n";
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  vcures_rvalid_" . $i . ", vcures_raddr_" . $i . ", vcures_rdata_" . $i . ", vcures_rready_" . $i . ",\n";
}
print $fh "\n";
  print $fh "  vcures_wen, vcures_waddr, vcures_wdata\n";
print $fh ");\n";
print $fh "\n";
...
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "input wire vcures_rvalid_" . $i . ";\n";
  print $fh "input wire [", $sram_addr_bits_all-1, ":0] vcures_raddr_" . $i . ";\n";
  print $fh "output wire [", $width-1, ":0] vcures_rdata_" . $i . ";\n";
  print $fh "output wire vcures_rready_" . $i . ";\n";
  print $fh "\n";
}
print $fh "input wire vcures_wen;\n";
print $fh "input wire [", $sram_addr_bits_all-1, ":0] vcures_waddr;\n";
print $fh "input wire [", $width-1, ":0] vcures_wdata;\n";
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
...
print $fh "assign waddr_high_bits = vcures_waddr[" . ($sram_addr_bits_all-1) . ":" . ($sram_addr_bits_all-$write_highaddr_bits) . "];\n";
print $fh "\n";
  ...
  if ($i < $bank) {
    for (my $j = 0; $j < $bank; $j++) {
      if ($j != $i) {
        ...
        print $fh "      wen_" . $i . " = vcures_wen;\n";
        print $fh "      waddr_" . $i . " = vcures_waddr[" . ($sram_addr_bits-1) . ":0];\n";
        print $fh "      wdata_" . $i . " = vcures_wdata;\n";
      }
    }
  }
  else {
    for (my $j = 0; $j < $bank; $j++) {
      if ($j != ($i - $bank)) {
        ...
        print $fh "      wen_" . ($i - $bank) . " = vcures_wen;\n";
        print $fh "      waddr_" . ($i - $bank) . " = vcures_waddr[" . ($sram_addr_bits-1) . ":0];\n";
        print $fh "      wdata_" . ($i - $bank) . " = vcures_wdata;\n";
      }
    }
  }
  print $fh "    end\n";
...
print $fh "vcures_arbiter u_arbiter(\n";
print $fh "  .clk(clk),\n";
print $fh "  .rst_n(rst_n),\n";
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  .rvalid_" . $i . "(vcures_rvalid_" . $i . "),\n";
}
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
...
for (my $i = 0; $i < $bank; $i++) {
  if ($i == $bank - 1) {
    print $fh "  .rready_" . $i . "(vcures_rready_" . $i . ")\n";
  }
  else {
    print $fh "  .rready_" . $i . "(vcures_rready_" . $i . "),\n";
  }
...
for (my $i = 0; $i < $bank; $i++) {
  print $fh "sram_2048x512 u_ram_bank_" . $i . "(\n";
  print $fh "  .w_clk(clk),\n";
