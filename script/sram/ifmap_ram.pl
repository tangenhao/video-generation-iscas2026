...
print "==== INFO: Generating ifmap_ram ==== \n";
print "width: ", $width, "\n";
print "depth: ", $depth, "\n";
print "bank: ", $bank, "\n";
...
print $fh "module ifmap_ram(\n";
print $fh "  clk, rst_n,\n";
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  ifmap_rvalid_" . $i . ", ifmap_raddr_" . $i . ", ifmap_rdata_" . $i . ", ifmap_rready_" . $i . ", ifmap_rsparse_" . $i . ",\n";
}
print $fh "\n";
print $fh "  ifmap_wen, ifmap_waddr, ifmap_wdata\n";
print $fh ");\n";
print $fh "\n";
...
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "input wire ifmap_rvalid_" . $i . ";\n";
  print $fh "input wire [", $sram_addr_bits_all-1, ":0] ifmap_raddr_" . $i . ";\n";
  print $fh "output wire [", $width*$bank-1, ":0] ifmap_rdata_" . $i . ";\n";
  print $fh "output wire ifmap_rready_" . $i . ";\n";
  print $fh "input wire [1:0] ifmap_rsparse_" . $i . ";\n";
  print $fh "\n";
}
print $fh "input wire ifmap_wen;\n";
print $fh "input wire [", $sram_addr_bits_all-1, ":0] ifmap_waddr;\n";
print $fh "input wire [", $width-1, ":0] ifmap_wdata;\n";
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
...
print $fh "assign waddr_high_bits = ifmap_waddr[" . ($sram_addr_bits_all-1) . ":" . ($sram_addr_bits_all-$write_highaddr_bits) . "];\n";
print $fh "\n";
  ...
  if ($i < $bank) {
    for (my $j = 0; $j < $bank; $j++) {
      if ($j != $i) {
        ...
        print $fh "      wen_" . $i . " = ifmap_wen;\n";
        print $fh "      waddr_" . $i . " = ifmap_waddr[" . ($sram_addr_bits-1) . ":0];\n";
        print $fh "      wdata_" . $i . " = ifmap_wdata;\n";
      }
    }
  }
  else {
    for (my $j = 0; $j < $bank; $j++) {
      if ($j != ($i - $bank)) {
        ...
        print $fh "      wen_" . ($i - $bank) . " = ifmap_wen;\n";
        print $fh "      waddr_" . ($i - $bank) . " = ifmap_waddr[" . ($sram_addr_bits-1) . ":0];\n";
        print $fh "      wdata_" . ($i - $bank) . " = ifmap_wdata;\n";
      }
    }
  }
  print $fh "    end\n";
...
print $fh "ifmap_read_arbiter u_arbiter(\n";
print $fh "  .clk(clk),\n";
print $fh "  .rst_n(rst_n),\n";
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  .rvalid_" . $i . "(ifmap_rvalid_" . $i . "),\n";
}
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  .addr_" . $i . "(ifmap_raddr_" . $i . "),\n";
}
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
...
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  .data_" . $i . "(ifmap_rdata_" . $i . "),\n";
}
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  print $fh "  .rready_" . $i . "(ifmap_rready_" . $i . "),\n";
}
print $fh "\n";
for (my $i = 0; $i < $bank; $i++) {
  if ($i == $bank - 1) {
    print $fh "  .rsparse_" . $i . "(ifmap_rsparse_" . $i . ")\n";
  }
  else {
    print $fh "  .rsparse_" . $i . "(ifmap_rsparse_" . $i . "),\n";
  }
