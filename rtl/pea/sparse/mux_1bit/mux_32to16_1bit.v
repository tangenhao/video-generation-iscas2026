...
mux_32to1_1bit u_mux_0 (
  .mask(mask_0_sel),
  .data(data),
  .out(out[0])
);

mux_32to1_1bit u_mux_1 (
  .mask(mask_1_sel),
  .data(data),
  .out(out[1])
);

mux_32to1_1bit u_mux_2 (
  .mask(mask_2_sel),
  .data(data),
  .out(out[2])
);

mux_32to1_1bit u_mux_3 (
  .mask(mask_3_sel),
  .data(data),
  .out(out[3])
);

mux_32to1_1bit u_mux_4 (
  .mask(mask_4_sel),
  .data(data),
  .out(out[4])
);

mux_32to1_1bit u_mux_5 (
  .mask(mask_5_sel),
  .data(data),
  .out(out[5])
);

mux_32to1_1bit u_mux_6 (
  .mask(mask_6_sel),
  .data(data),
  .out(out[6])
);

mux_32to1_1bit u_mux_7 (
  .mask(mask_7_sel),
  .data(data),
  .out(out[7])
);

mux_32to1_1bit u_mux_8 (
  .mask(mask_8_sel),
  .data(data),
  .out(out[8])
);

mux_32to1_1bit u_mux_9 (
  .mask(mask_9_sel),
  .data(data),
  .out(out[9])
);

mux_32to1_1bit u_mux_10 (
  .mask(mask_10_sel),
  .data(data),
  .out(out[10])
);

mux_32to1_1bit u_mux_11 (
  .mask(mask_11_sel),
  .data(data),
  .out(out[11])
);

mux_32to1_1bit u_mux_12 (
  .mask(mask_12_sel),
  .data(data),
  .out(out[12])
);

mux_32to1_1bit u_mux_13 (
  .mask(mask_13_sel),
  .data(data),
  .out(out[13])
);

mux_32to1_1bit u_mux_14 (
  .mask(mask_14_sel),
  .data(data),
  .out(out[14])
);

mux_32to1_1bit u_mux_15 (
  .mask(mask_15_sel),
  .data(data),
  .out(out[15])
);
