...
class SoftmaxEmulator():
    ...
    def __init__(self, seq_len: int, d_model: int, num_cores: int,
        ...
        if debug:
            print(f"Softmax computing, number of iterations {self.iterations}")
    ...
    def Softmax(self):
        ...
        if self.debug:
            print(f"Config Cycles: {total_cycles}")
        ...
        if self.debug:
            print(f"Exp Cycles: {self.seq_len * self.max_n_groups * 22}")
        ...
        if self.debug:
            print(f"Reduce Sum Cycles: {self.seq_len * self.max_n_groups * 32}")
            
        # div
        total_cycles += 32 * 10

        if self.debug:
            print(f"Div Cycles: {32 * 10}")
        ...
        if self.debug:
           print(f"Mul Cycles: {self.seq_len * self.max_n_groups * 7}")

        if self.debug:
            print(f"Softmax Cycles: {total_cycles}")
    ...
    def Run(self):
        ...
        for _ in range(self.iterations):
            total_time += max(self.LoadPsum(), self.Softmax(), self.Store())
        
        return total_time
      
softmax_t = SoftmaxEmulator(128, 64, 1, 25, 0.5, 800, True)
print(softmax_t.Run())