...
# Ensure data_gen can be imported. If it's data_gen.py, then "import data_gen" is fine.
# If data_gen contains functions directly, you might need "from data_gen import load_tensor_from_bin"
# Assuming "import data_gen" and then using "data_gen.load_tensor_from_bin"
try:
    import data_gen 
...
def main():
    parser = argparse.ArgumentParser(description="Compare Softmax FP32 outputs (PyTorch vs C++)")
    ...
    parser.add_argument("--oc_group_size", type=int, required=True, help="Size of oc_group (feature dim, softmax is over this)")
    ...
    shape = (args.oc_group, args.seq_len, args.oc_group_size)
    dtype = np.float32

    os.makedirs(args.charts_dir, exist_ok=True)
    # Ensure directory for C++ output exists, as C++ sim might not create it
    # However, if cpp_output_path could be in a non-existent dir passed from outside, this is safer.
    # Let's assume cpp_output_path is a full file path and its directory is already handled by run_op_validation.py.
    # os.makedirs(os.path.dirname(args.cpp_output_path), exist_ok=True) # Commenting out, should be handled by main runner

    print(f"--- Softmax (FP32) Output Comparison ---")
    print(f"Shape: {shape}, Dtype: {dtype}")
    print(f"Using dimensions: oc_group={args.oc_group}, seq_len={args.seq_len}, oc_group_size={args.oc_group_size}")
    ...
    print(f"C++ output file to load: {args.cpp_output_path}")
    ...
    # --- Step 2: Run C++ simulator --- (REMOVED)
    ...
    if not os.path.exists(args.cpp_output_path):
        print(f"\nError: C++ output file not found (expected to be pre-generated): {args.cpp_output_path}")
        sys.exit(1)
    ...
    plot_scatter_comparison(
        cpp_output_tensor, 
        ref_output_tensor, 
        title="Softmax FP32: C++ Output vs. PyTorch Reference",
        save_path=os.path.join(args.charts_dir, "softmax_fp32_scatter_comparison.png")
    )
    print(f"Scatter comparison plot saved to {os.path.join(args.charts_dir, 'softmax_fp32_scatter_comparison.png')}")

    plot_error_histogram(
        metrics['absolute_error_array'], 
        title="Softmax FP32: Absolute Error Distribution",
        save_path=os.path.join(args.charts_dir, "softmax_fp32_abs_error_hist.png")
    )
    print(f"Absolute error histogram saved to {os.path.join(args.charts_dir, 'softmax_fp32_abs_error_hist.png')}")

    plot_error_histogram(
        metrics['relative_error_array'], 
        title=f"Softmax FP32: Relative Error Distribution (epsilon={metrics['epsilon_for_relative_error']:.1e})",
        save_path=os.path.join(args.charts_dir, "softmax_fp32_rel_error_hist.png")
    )
    print(f"Relative error histogram saved to {os.path.join(args.charts_dir, 'softmax_fp32_rel_error_hist.png')}")
    ...
    print("\nSoftmax comparison script finished.")
