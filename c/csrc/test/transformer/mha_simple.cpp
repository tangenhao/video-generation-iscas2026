#include "transformer/mha_simple.h"
...
int main()
{
  ...
  /*                                               Instructions                                               */
  /* -------------------------------------------------------------------------------------------------------- */
  using mha_t = transformer::mha::MultiHeadAttentionSimpleOp<true, false>;
  mha_t           mha_op;
  mha_t::Argument args        = {seq_len,
                                 d_model,
                                 head_num,
                                 INPUT_ADDR,
                                 QUERY_WEIGHT_ADDR,
  ...
  opcode.resize(vcucode_ddr_lines * 8, 0);
  ...
  if (update_reference) {
    ...
    gemm_sim_dbg_t::Arguments args_query_sim = {query_temp, input, weight_query, seq_len, 4, 4};
    ...
    gemm_sim_t::Arguments args_key_sim = {key_temp, input, weight_key, seq_len, 4, 4};
    ...
    gemm_sim_t::Arguments args_value_sim = {value_temp, input, weight_value, seq_len, 4, 4};
    ...
    for (int head = 0; head < head_num; head++) {
      ...
      gemm_sim_t::Arguments args_output_sim = {output_head, probe_head, value_head, 32, 1, 1};
    ...
    gemm_sim_t::Arguments args_output_sim = {output, output_temp, weight_output, seq_len, 4, 4};
    gemm_sim_op(args_output_sim);
