...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<0, 0, 0, 0, kInt8, kInt4, kInt32, kInt32, true>;
  ...
  using gemm_sim_t = compute_model::gemm::GemmSim<0, 0, false, false, int8_t, int4_t, int32_t, int32_t, true>;
  using namespace compute_model::tensor;

  auto ifmap  = randn<int8_t>({k_group, m, k_group_size}, kHalf, -128.0f, 127.0f, 0);
  auto weight = randn<int4_t>({n_group, k_group / 2, n_group_size, k_group_size * 2}, kHalf, -8.0f, 7.0f, 100);
