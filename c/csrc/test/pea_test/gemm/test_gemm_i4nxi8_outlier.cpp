...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<0, true, 0, true, kInt4, kInt8, kFloat32, kFloat32, true>;
  ...
  auto weight        = randn<int8_t>({n_group, k_group, n_group_size, k_group_size}, kHalf, -128.0f, 127.0f, 100);
  ...
  auto outlier_index = randn<int8_t>({k_group / 2, m, k_group_size * 2}, kHalf, 0, 2, 0);
  auto outlier_scale = randn<half>({m}, kHalf, -1.0f, 1.0f, 0);

  using gemm_sim_t               = compute_model::gemm::GemmSim<0, true, false, true, int4_t, int8_t, float, float, true>;
  ...
  auto outlier_index_pad_temp = zeros<int8_t>({k_group / 2, m, k_group_size}, kInt8);
  ...
  // auto outlier_index_pad = zeros<int8_t>({k_group, m, 128}, kInt8);
  // for (int i = 0; i < k_group; i++) {
  //   for (int j = 0; j < m; j++) {
  //     for (int l = 0; l < 128; l++) {
  //       outlier_index_pad[i * m * 128 + j * 128 + l] =
  //         outlier_index_pad_temp[i * m * k_group_size * 2 + j * k_group_size * 2 + l];
  //     }
  //   }
  // }
  ...
  common::file_utils::saveCharArrayToFormattedTextFile("../../sim/memory/outlier_index.txt",
                                                       reinterpret_cast<char*>(outlier_index_pad_temp.data_ptr()),
