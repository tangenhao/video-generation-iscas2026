...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<0, 0, 0, 0, kInt8, kHalf, kFloat32, kFloat32, true>;
  ...
  using gemm_sim_t = compute_model::gemm::GemmSim<0, 0, false, false, int8_t, half, float, float, true>;

  auto ifmap  = randn<int8_t>({k_group / 2, m, k_group_size * 2}, kHalf, -128.0f, 127.0f, 0);
  auto weight = randn<half>({n_group, k_group, n_group_size, k_group_size}, kHalf, -8.0f, 7.0f, 100);
