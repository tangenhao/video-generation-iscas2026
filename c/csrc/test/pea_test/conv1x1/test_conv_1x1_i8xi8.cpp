...
int main(int argc, const char** argv)
{
  ...
  auto ifmap  = randn<int8_t>({ic_group, h, w, ic_group_size}, kHalf, -128, 0, 0);
  auto weight = randn<int8_t>({oc_group, ic_group, kh, kw, oc_group_size, ic_group_size}, kHalf, 0, 127, 100);
  auto ofmap  = zeros<int32_t>({oc_group, ofmap_h, ofmap_w, oc_group_size}, kFloat32);

  using conv2d_sim_t                  = compute_model::conv2d::Conv2dSim<0, false, false, false, int8_t, int8_t, int32_t, int32_t, true>;
