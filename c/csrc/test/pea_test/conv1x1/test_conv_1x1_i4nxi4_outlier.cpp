...
int main(int argc, const char** argv)
{
  ...
  using conv2d_t           = pea::Conv2dOp<0, true, false, true, kInt4, kInt4, kFloat32, kFloat32, true>;
  ...
  auto ifmap         = randn<int4_t>({ic_group, h, w, ic_group_size}, kHalf, -8, 7, 0);
  auto weight        = randn<int4_t>({oc_group, ic_group, kh, kw, oc_group_size, ic_group_size}, kHalf, -8, 7, 100);
  ...
  auto outlier_scale = randn<half>({h, w}, kHalf, -1.0f, 1.0f, 200);

  using conv2d_sim_t = compute_model::conv2d::Conv2dSim<0, true, false, true, int4_t, int4_t, float, float, true>;

  conv2d_sim_t::Arguments conv2d_args = {ofmap,       ifmap,         weight,        stride_h,     stride_w, pad_h, pad_w, dilation_h,
                                         dilation_w,  ifmap_block_h, ifmap_block_w, kh,           kw,       1,     1,     Tensor<int8_t>(),
                                         ifmap_scale, weight_scale,  outlier_index, outlier_scale};
