...
int main(int argc, const char** argv)
{
  ...
  using conv2d_t           = pea::Conv2dOp<SPARSE_ENABLE, 0, false, false, kInt4, kInt8, kFloat32, kFloat32, true>;
  ...
  using conv2d_sim_t = compute_model::conv2d::Conv2dSim<SPARSE_ENABLE, 0, false, false, int4_t, int8_t, float, float, true>;
  ...
  auto outlier_index_pad = zeros<int8_t>({ic_group / 2, h, w, ic_group_size * 2}, kInt8);
  for (int i = 0; i < ic_group / 2; i++) {
    for (int k = 0; k < h; k++) {
      for (int l = 0; l < w; l++) {
        for (int j = 0; j < ic_group_size * 2 / 8; j++) {
          int8_t num = 0;
          for (int c = 0; c < 8; ++c) {
            num |=
              (outlier_index[i * h * w * (ic_group_size * 2) + k * w * (ic_group_size * 2) + l * (ic_group_size * 2) + j * 8 + c] << c);
          }
