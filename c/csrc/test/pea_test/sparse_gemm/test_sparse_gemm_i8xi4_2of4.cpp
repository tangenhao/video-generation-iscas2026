...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<SPARSE_ENABLE, 0, 0, 0, kInt8, kInt4, kInt32, kInt32, false>;
  ...
  auto ifmap        = randn<int8_t>({k_group, m, k_group_size}, kBfloat16, -128.0f, 127.0f, 0);
  auto weight_dense = randn<int4_t>({n_group, k_group / 2, n_group_size, k_group_size * 2}, kBfloat16, -8.0f, 7.0f, 100);
  auto ofmap        = zeros<int32_t>({n_group, m, n_group_size}, kFloat32);
  ...
  using gemm_sim_t               = compute_model::gemm::GemmSim<SPARSE_ENABLE, false, false, false, int8_t, int4_t, int32_t, int32_t, true>;
