...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<SPARSE_ENABLE, 0, 0, 0, kInt8, kBfloat16, kFloat32, kFloat32, false>;
  ...
  auto ifmap        = randn<int8_t>({k_group / 2, m, k_group_size * 2}, kHalf, -128.0f, 127.0f, 0);
  auto weight_dense = randn<bfloat16>({n_group, k_group, n_group_size, k_group_size}, kBfloat16, -1.0f, 1.0f, 100);
  ...
  using gemm_sim_t               = compute_model::gemm::GemmSim<SPARSE_ENABLE, false, false, false, int8_t, bfloat16, float, float, true>;
