...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<SPARSE_ENABLE, 0, false, true, kInt8, kInt4, kFloat32, kFloat32, false>;
  ...
  auto ifmap         = randn<int8_t>({k_group, m, k_group_size}, kHalf, -128.0f, 127.0f, 0);
  auto weight_dense  = randn<int4_t>({n_group, k_group / 2, n_group_size, k_group_size * 2}, kHalf, -8.0f, 7.0f, 100);
  auto ifmap_mask    = GenIfmapMask(n_group, k_group / 2, n_group_size, k_group_size * 2, SPARSE_ENABLE, 0);
  ...
  using gemm_sim_t               = compute_model::gemm::GemmSim<SPARSE_ENABLE, false, false, true, int8_t, int4_t, float, float, true>;
  ...
  auto outlier_index_pad = zeros<int8_t>({k_group, m, k_group_size}, kInt8);
  for (int i = 0; i < k_group; i++) {
    for (int j = 0; j < m; j++) {
      for (int l = 0; l < k_group_size / 8; l++) {
        ...
        outlier_index_pad[i * m * k_group_size + j * k_group_size + l] = num;
      }
    }
  }
