...
int main(int argc, const char** argv)
{
  ...
  using gemm_t           = pea::GemmOp<SPARSE_ENABLE, 0, false, true, kInt4, kInt8, kFloat32, kFloat32, false>;
  ...
  auto ifmap         = randn<int4_t>({k_group / 2, m, k_group_size * 2}, kHalf, -8.0f, 7.0f, 0);
  auto weight_dense  = randn<int8_t>({n_group, k_group, n_group_size, k_group_size}, kHalf, -128.0f, 127.0f, 100);
  ...
  auto outlier_index = randn<int8_t>({k_group / 2, m, k_group_size * 2}, kInt8, 0, 2, 0);
  ...
  using gemm_sim_t               = compute_model::gemm::GemmSim<SPARSE_ENABLE, false, false, true, int4_t, int8_t, float, float, true>;
  gemm_sim_t::Arguments args_sim = {ofmap,
                                    ifmap_transform,
  ...
  auto outlier_index_pad = zeros<int8_t>({k_group / 2, m, k_group_size * 2}, kInt8);
  for (int i = 0; i < k_group / 2; i++) {
    for (int j = 0; j < m; j++) {
      for (int l = 0; l < k_group_size * 2 / 8; l++) {
        int8_t num = 0;
        for (int c = 0; c < 8; ++c) {
          num |= (outlier_index[i * m * k_group_size * 2 + j * k_group_size * 2 + l * 8 + c] << c);
        }
        outlier_index_pad[i * m * k_group_size * 2 + j * k_group_size * 2 + l] = num;
      }
    }
  }
  ...
  auto ifmap_mask_compress = zeros<int8_t>({n_group, (k_group / sparse_ratio), n_group_size * (k_group_size / 8) * sparse_ratio}, kInt8);
