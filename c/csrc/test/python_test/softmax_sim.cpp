...
#include "compute_model/transformer/softmax.h" // Added for the library softmax function
#include "common/type_utils.h" // For DType

#include <fstream>   // For std::ifstream, std::ofstream
#include <string>
#include <vector>
#include <cmath> // For std::log2, std::exp
#include <iostream>
#include <stdexcept> // For std::runtime_error
#include <iomanip>   // For std::fixed, std::setprecision if needed for debug
...
int main(int argc, char* argv[]) {
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " <oc_group> <seq_len> <oc_group_size> <input_bin_path> <output_bin_path>" << std::endl;
        return 1;
    }

    // Parse command-line arguments
    const int oc_group_softmax = std::atoi(argv[1]);
    const int seq_len_softmax = std::atoi(argv[2]);
    const int oc_group_size_softmax = std::atoi(argv[3]);
    const std::string input_path = argv[4];
    const std::string output_path = argv[5];

    if (oc_group_softmax <= 0 || seq_len_softmax <= 0 || oc_group_size_softmax <= 0) {
        std::cerr << "Error: Dimensions must be positive." << std::endl;
        return 1;
    }

    std::cout << "Softmax C++ Simulation (FP32)" << std::endl;
    std::cout << "Parsed params: oc_group (batch_size)=" << oc_group_softmax 
              << ", seq_len=" << seq_len_softmax 
              << ", oc_group_size (d_model/features)=" << oc_group_size_softmax << std::endl;
    
    std::vector<int> shape = {oc_group_softmax, seq_len_softmax, oc_group_size_softmax};
    DType dtype = DType::kFloat32; // Softmax operates on FP32
    ...
    try {
        ...
        size_t num_input_elements = static_cast<size_t>(oc_group_softmax) * seq_len_softmax * oc_group_size_softmax;
        ...
        // --- Softmax Computation using softmax.h ---
        std::cout << "\nCalling compute_model::transformer::softmax::compute_softmax function..." << std::endl;
        auto output_tensor = compute_model::transformer::softmax::compute_softmax<float>(input_tensor);
        std::cout << "Softmax computation using library function complete." << std::endl;
        std::cout << "Output tensor shape: {" << output_tensor.shape()[0] << ", "
                  << output_tensor.shape()[1] << ", " << output_tensor.shape()[2] << "}" << std::endl;

        // 7. Save output tensor
        ...
        if (output_ptr && num_output_elements > 0) {
            ...
            // Vector is already empty, which is fine for an empty tensor.
             std::cout << "Output tensor is empty (0 elements)." << std::endl;
        } else { // Should not happen if num_output_elements > 0 but output_ptr is null
        ...
        std::cout << "Softmax C++ simulation complete. Output saved to " << output_path << std::endl;
