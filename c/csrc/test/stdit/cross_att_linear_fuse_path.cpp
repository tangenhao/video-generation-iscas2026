...
// fuse_path: bias_add -> gate_msa * self_attention_output->residual add->cross-attention（linear）

// vcu: bias_add -> gate_msa * self_attention_output->residual add
// 1. bias_add->mul（gate_msa）->add（residual_add）-> psum  (因为quant需要二次复用)
// 2. quant_1: abs->redmax, ascale=mulc(max*(1/127),reg1), mul(ascale*wscale) -> scale(pea)
// 3. rec(ascale,reg1)->mul(psum(x_fp)*reg1, x*ascale)->clamp->round->qact(pea)

// pea: x*wq, x*wk, x*wv -> dequant

// store: fp_act, 按照当前是spatial还是temporal进行分配地址
