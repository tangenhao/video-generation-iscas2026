...
int main(int argc, const char** argv)
{
  ...
  auto data = randn<int8_t>({32, 32}, kInt8, -1.0f, 1.0f, 0);

  common::file_utils::saveCharArrayToFormattedTextFile(
    psum_file.c_str(), reinterpret_cast<char*>(data.data_ptr()), data.numel() * sizeof(uint8_t), 32, true);

  std::vector<insn::instruction> insn_series;

  insn_series.push_back(insn::load_iteration_2(data_in_ddr_base_addr, 32 * 1 - 1, 0, 0, 0, MASTER_PSUM_ADDR + 8192 * 1, 0));

  using vcu_transpose_t           = vcu::VcuTranspose;
  vcu_transpose_t::Arguments args = {1, psum_sram_read_addr, psum_sram_write_addr};
  ...
  insn_series.push_back(insn::store_iteration_2(data_out_ddr_base_addr, 32 * 1 - 1, 0, 0, 0, MASTER_PSUM_ADDR, 1));
  ...
  common::file_utils::saveCharArrayToFormattedTextFile(
    ofmap_file.c_str(), ofmap.data_ptr<char>(), ofmap.numel() * sizeof(uint8_t), 32, true);
