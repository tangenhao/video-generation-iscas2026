...
int main(int argc, const char** argv)
{
  ...
  auto data = randn<float>({32, 32}, kFloat32, -1.0f, 1.0f, 0);
  ...
  insn_series.push_back(insn::load_iteration_2(data_in_ddr_base_addr, 32 * 4 - 1, 0, 0, 0, MASTER_PSUM_ADDR + 8192 * 4, 0));
  ...
  insn_series.push_back(insn::store_iteration_2(data_out_ddr_base_addr, 32 * 4 - 1, 0, 0, 0, MASTER_PSUM_ADDR, 1));
  ...
  common::file_utils::saveCharArrayToFormattedTextFile(ofmap_file.c_str(), ofmap.data_ptr<char>(), ofmap.numel() * sizeof(float), 32, true);
