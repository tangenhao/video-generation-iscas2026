...
#include "compute_model/sample/avgpool.h"
#include "sample/avgpool.h"
...
int main(int argc, const char** argv)
{
  ...
  int dilation_h    = 1;
  int dilation_w    = 1;
  int ifmap_block_h = 14;
  int ifmap_block_w = 14;
  ...
  using avgpool_t                   = vcu::AvgpoolOp<kBfloat16, kBfloat16, false>;
  avgpool_t::Arguments maxpool_args = {h,
                                       w,
                                       kh,
                                       kw,
  ...
  auto data_in  = randn<bfloat16>({oc_group, h, w, oc_group_size}, kBfloat16, -1.0f, 1.0f, 0);
  auto data_out = zeros<bfloat16>({oc_group, 7, 7, oc_group_size}, kBfloat16);

  using avgpool_sim_t           = compute_model::sample::Avgpool2dSim<bfloat16, bfloat16, true>;
  avgpool_sim_t::Arguments args = {data_out, data_in, {kh, kw}, {stride_h, stride_w}, {pad_h, pad_w}, {dilation_h, dilation_w}};
  avgpool_sim_t            avgpool_sim;
  avgpool_sim(args);

  common::file_utils::saveCharArrayToFormattedTextFile(
    psum_file.c_str(), reinterpret_cast<char*>(data_in.data_ptr()), data_in.numel() * sizeof(bfloat16), 32, true);
