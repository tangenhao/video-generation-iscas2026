...
int main(int argc, const char** argv)
{
  ...
  auto data_out = compute_model::function::fast_sigmoid(data_in);
  ...
  auto vcucode_series = vcu::asm_vcu_op({"fastsig psum, reg0"});
  ...
  insn_series.push_back(insn::load_iteration_2<0>(sigmoid_lut_ddr_base_addr, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
