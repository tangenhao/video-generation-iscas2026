...
int main(int argc, const char** argv)
{
  ...
  // common::file_utils::saveCharArrayToFormattedTextFile("../../sim/memory/shuffle_in.txt",
  //                                                      reinterpret_cast<char*>(data_in.data_ptr()),
  //                                                      data_in.numel() * sizeof(float),
  //                                                      256,
  //                                                      true);

  auto out_shuffle = zeros<float>({oc_group, num_data, 32}, kFloat32);
  out_shuffle      = compute_model::function::shuffle(data_in);

  // common::file_utils::saveCharArrayToFormattedTextFile("../../sim/memory/shuffle_out.txt",
  //                                                      reinterpret_cast<char*>(out_shuffle.data_ptr()),
  //                                                      out_shuffle.numel() * sizeof(float),
  //                                                      256,
  //                                                      true);
  auto data_out = out_shuffle + 0.12548828125;
  ...
  auto vcucode_series = vcu::asm_vcu_op({"copy psum, reg1", "shfl 0x02", "addc reg1, reg7, 0.12548828125"});
