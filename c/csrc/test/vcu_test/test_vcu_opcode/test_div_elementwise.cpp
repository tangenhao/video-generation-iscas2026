...
int main(int argc, const char** argv)
{
  ...
  uint64_t sincos_lut_ddr_base_addr = SINCOS_LUT_ADDR;
  ...
  if (cmd.check_cmd_line_flag("help")) {
    std::cout << "Usage: " << argv[0] << "\nOptions:\n"
              ...
              << "  --sincos_lut_base_addr=<long>   Base address of the sin_cos_lut in hex, default is " << sincos_lut_ddr_base_addr << "\n"
  ...
  cmd.get_cmd_line_argument("sincos_lut_base_addr", sincos_lut_ddr_base_addr);
  ...
  auto data_in          = randn<float>({oc_group, num_data, 32}, kFloat32, -100.0f, 100.0f, 0);
  auto data_elementwise = randn<float>({oc_group, num_data, 32}, kFloat32, -10.0f, 10.0f, 0);
  auto data_out         = data_in / data_elementwise;
  ...
  if (save_bin) {
    ...
    common::file_utils::saveCharArrayToBinFile(
      res_file.c_str(), reinterpret_cast<char*>(data_elementwise.data_ptr()), data_elementwise.numel() * sizeof(float));
    common::file_utils::saveCharArrayToBinFile(
    ...
    common::file_utils::saveCharArrayToFormattedTextFile(
      res_file.c_str(), reinterpret_cast<char*>(data_elementwise.data_ptr()), data_elementwise.numel() * sizeof(float), 32, true);
    common::file_utils::saveCharArrayToFormattedTextFile(
  ...
  auto vcucode_series = vcu::asm_vcu_op({"divm psum resadd, reg11"});
  ...
  insn_series.push_back(insn::load_iteration_2<0>(sincos_lut_ddr_base_addr, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
  ...
  insn_series.push_back(insn::load_iteration_3<0>(VCURES_ADDR,
                                                  w * 4 - 1,
                                                  seq_1_offset.first,
                                                  seq_1_offset.second,
                                                  h - 1,
