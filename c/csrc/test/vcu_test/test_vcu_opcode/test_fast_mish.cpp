...
int main(int argc, const char** argv)
{
  ...
  if (cmd.check_cmd_line_flag("help")) {
    std::cout << "Usage: " << argv[0] << "\nOptions:\n"
              ...
              << "  --mish_lut_ddr_base_addr=<long>   Base address of the sin_cos_lut in hex, default is " << mish_lut_ddr_base_addr << "\n"
  ...
  cmd.get_cmd_line_argument("mish_lut_ddr_base_addr", mish_lut_ddr_base_addr);
  ...
  auto data_out = compute_model::function::fast_mish(data_in);
  ...
  auto vcucode_series = vcu::asm_vcu_op({"fastmish psum, reg0"});
  ...
  insn_series.push_back(insn::load_iteration_2<0>(mish_lut_ddr_base_addr, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
