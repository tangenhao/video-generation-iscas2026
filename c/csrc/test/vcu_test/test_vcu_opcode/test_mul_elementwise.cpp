...
int main(int argc, const char** argv)
{
  ...
  uint64_t data_mul_ddr_base_addr   = VCURES_ADDR;
  ...
  auto vcucode_series = vcu::asm_vcu_op({
    "mul psum resadd, reg0",  // elementwise mul - psum * resadd -> reg0
  });
  ...
  // 加载第二个输入数据
  insn_series.push_back(insn::load_iteration_2<0>(
    data_mul_ddr_base_addr, seq_len * bytes_input * oc_group_size / 32 - 1, seq_1_offset.first, seq_1_offset.second, oc_group - 1, MASTER_VCURES_ADDR, 0));
  ...
  // 执行mul elementwise操作
  vcu_t::Arguments mul_args = {0b100,                   // psum_data_type
  ...
  auto mul_insns = vcu_op(mul_args);
  insn_series.insert(insn_series.end(), mul_insns.begin(), mul_insns.end());
