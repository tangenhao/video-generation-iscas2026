...
int main(int argc, const char** argv)
{
  ...
  std::cout << std::hex << (*(uint32_t*)(&data_in[64])) << std::endl;
  auto data_out = data_in * (-log2(exp(1.0f)));
  std::cout << std::hex << (*(uint32_t*)(&data_out[64])) << std::endl;
  ...
  auto vcucode_series = vcu::asm_vcu_op(
    {"copy psum, reg1", "mulc psum, reg0, -1.442695", "exp2 reg0, reg0", "addc reg0, reg0, 1.0", "rec reg0, reg0", "mul reg1 reg0, reg0"},
    true);
