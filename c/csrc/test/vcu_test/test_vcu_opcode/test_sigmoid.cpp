...
int main(int argc, const char** argv)
{
  ...
  std::cout << "seed: " << random_num << std::endl;

  auto data_in = randn<float>({oc_group, num_data, 32}, kFloat32, -10000.0f, 10000.0f, random_num);

  auto data_out = compute_model::function::sigmoid(data_in);
  ...
  auto vcucode_series = vcu::asm_vcu_op({"mulc psum, reg0, -1.442695", "exp2 reg0, reg0", "addc reg0, reg0, 1.0", "rec reg0, reg0"});
