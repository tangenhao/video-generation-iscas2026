...
static inline float asfloata(uint32_t i)
{
  union {
    uint32_t i;
    float    f;
...
int main(int argc, const char** argv)
{
  ...
  auto data_out = data_in * asfloata(0xc1e5c600);
  data_out      = data_out + asfloata(0x3feaa000);
  data_out      = data_out / asfloata(0x419dc6a8);
  data_out      = data_out * asfloata(0x3f199900);

  for (unsigned int it = 0; it < loop_times; it++) {
    data_out = data_out * asfloata(0xc1e5c600);
    data_out = data_out + asfloata(0x3feaa000);
    data_out = data_out / asfloata(0x419dc6a8);
  }

  data_out = data_out + asfloata(0xc1455555);
  ...
  /*                                                opcode gen                                                */
  /* -------------------------------------------------------------------------------------------------------- */
  // "loop times, initial, end, loop_addr"
  auto   vcucode_series    = vcu::asm_vcu_op({"copy psum, reg1",
                                         "mulc reg1, reg3, -28.7216796875",
                                         "addc reg3, reg2, 1.8330078125",
                                         "divc reg2, reg1, 19.722000122070312",
                                         "mulc reg1, reg1, 0.5999908447265625",
                                         "loop 1, 1, 3, 5",
                                         "addc reg1, reg2, -12.333333015441895"});
