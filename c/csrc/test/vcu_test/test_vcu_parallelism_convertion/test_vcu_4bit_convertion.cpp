...
int main(int argc, const char** argv)
{
  ...
  auto data = randn<int4_t>({oc_group, num_data, 64}, kHalf, -8.0f, 7.0f, 0);
  ...
  auto data_reshape = zeros<int4_t>({oc_group / 2, num_data, 64}, kInt4);
  ...
  vcu_convertion_t::Arguments args = {
    1, psum_sram_read_addr, ofmap_sram_write_addr, (uint64_t)num_data, (uint64_t)oc_group, (uint64_t)(oc_group / 2)};
  ...
  insn_series.push_back(insn::store_iteration_2<0>(
    data_out_ddr_base_addr, num_data - 1, seq_1_offset.first, seq_1_offset.second, oc_group / 2 - 1, MASTER_OFMAP_ADDR, 1));
