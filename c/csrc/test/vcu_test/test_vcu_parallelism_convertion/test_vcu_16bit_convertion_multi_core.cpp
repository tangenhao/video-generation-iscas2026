...
int main(int argc, const char** argv)
{
  ...
  auto data = randn<half>({oc_group, num_data, 32}, kHalf, -1.0f, 1.0f, 0);
  ...
  auto data_reshape = zeros<half>({oc_group * 2, num_data, 16}, kHalf);
  ...
  for (int i = 0; i < oc_group; ++i) {
    for (int j = 0; j < num_data; ++j) {
      for (int k = 0; k < 32; ++k) {
        ...
        int dst_idx;
        if (k < 16) {
          dst_idx = 2 * i * num_data * 16 + j * 16 + k;
        }
        else {
          dst_idx = (2 * i + 1) * num_data * 16 + j * 16 + k - 16;
        }
