...
int main(int argc, const char** argv)
{
  ...
  using conv2d_t                         = npu_u::Conv2dOp<0, 0, 0, 0, kBfloat16, kBfloat16, kFloat32, kHalf, true>;
  ...
  conv2d_t::Arguments       args         = {h,
                              ...
                              {"add psum para, reg6",
                               ...
                               "config reg8, 0.0",
                               "mulc reg11, reg7, 0.1",
                               "compgeq reg11, reg8, reg11, reg7, reg7"}};
  conv2d_t                  conv2d_op;
  ...
  auto ifmap     = randn<bfloat16>({ic_group, h, w, ic_group_size}, kHalf, -4.0f, 4.0f, 0);
  auto weight    = randn<bfloat16>({oc_group, ic_group, kh, kw, oc_group_size, ic_group_size}, kBfloat16, -4.0f, 4.0f, 100);
  ...
  // bias---------------------------------------------------------------------------
  auto para = randn<float>({oc_group, 1, 1, oc_group_size}, kFloat32, -8.0f, 8.0f, 6);

  common::file_utils::saveCharArrayToFormattedTextFile(
    para_file.c_str(), reinterpret_cast<char*>(para.data_ptr()), para.numel() * sizeof(float), 32, true);

  //--------------------------------------------------------------
  using conv2d_sim_t                  = compute_model::conv2d::Conv2dSim<0, false, false, false, bfloat16, bfloat16, float, float, true>;
  ...
  common::file_utils::saveCharArrayToFormattedTextFile(
    "../../sim/memory/ofmap_mid_pea_out.txt", reinterpret_cast<char*>(pea_ofmap.data_ptr()), pea_ofmap.numel() * sizeof(float), 64, true);

  // common::file_utils::saveCharArrayToFormattedTextFile(
  //   "../../sim/memory/bias_in.txt", reinterpret_cast<char*>(para.data_ptr()), para.numel() * sizeof(float), 64,
  //   true);

  common::file_utils::saveCharArrayToFormattedTextFile(
    "../../sim/memory/bias_add_out.txt", reinterpret_cast<char*>(bias_add_out.data_ptr()), bias_add_out.numel() * sizeof(float), 64, true);

  auto resadd_fp = ToFloat32(resadd);
  common::file_utils::saveCharArrayToFormattedTextFile(
    "../../sim/memory/resadd_fp.txt", reinterpret_cast<char*>(resadd_fp.data_ptr()), resadd_fp.numel() * sizeof(float), 64, true);
  auto resadd_out = bias_add_out + resadd_fp;
  auto vcu_ofmap  = zeros<float>({oc_group, ofmap_h, ofmap_w, oc_group_size}, kFloat32);
  vcu_ofmap       = compute_model::function::leaky_relu(resadd_out, 0.1);

  auto vcu_ofmap_half = ToFloat16(vcu_ofmap);
  ...
  auto ofmap_reshape = ParallelismConvertion32to16(vcu_ofmap_half);
  ...
  common::file_utils::saveCharArrayToFormattedTextFile(
    ofmap_file.c_str(), (char*)ofmap_reshape.data_ptr(), ofmap_reshape.numel() * sizeof(half), 32, true);
