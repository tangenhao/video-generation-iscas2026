...
int main(int argc, const char** argv)
{
  ...
  vcu::vcu_exe_args         vcu_exe_args = {kHalf, 1, 1, 0, 4, 0, 0, 0, 0, 0, 0, 0};
  conv2d_t::Arguments       args         = {h,
                              ...
                              vcu_cfg_args,
                              vcu_exe_args,
                              {"add psum para, reg6", "add reg6 resadd, reg11", "fasttanh reg11, reg7"}};
  conv2d_t                  conv2d_op;
  ...
  auto resadd = randn<half>({oc_group, ofmap_h, ofmap_w, oc_group_size}, kHalf, -1.0f, 1.0f, 7);

  common::file_utils::saveCharArrayToFormattedTextFile(
    res_file.c_str(), reinterpret_cast<char*>(resadd.data_ptr()), resadd.numel() * sizeof(half), 32, true);
  ...
  common::file_utils::saveCharArrayToFormattedTextFile(
    "../../sim/memory/ofmap_mid_para_bias.txt", reinterpret_cast<char*>(para.data_ptr()), para.numel() * sizeof(float), 128, true);
  ...
  vcu_ofmap      = compute_model::function::fast_tanh(resadd_out);
  ...
  std::cout << "vcu_ofmap_bf16.size() = " << vcu_ofmap_bf16.numel() << std::endl;
