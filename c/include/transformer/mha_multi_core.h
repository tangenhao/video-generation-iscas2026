...
struct ConfigMHA {
  ...
  std::pair<std::vector<instruction>, std::vector<uint64_t>> operator()(const Argument& args)
  {
    ...
    this->set_vcucode(opcode, args);

    auto num_vcucodes = opcode.size();
    num_vcucodes      = round_off(num_vcucodes, 4);

    /** Load vcucode for each core */
    insn_series.push_back(synchronize_indie(
      3, 0b00000000000000000000000001010101, 0b00000000000000000000000001010101, 0b11111111000000000000000001010101, 0, 0));
    insn_series.push_back(synchronize_indie(
      3, 0b00000000000000000000000001010101, 0b00000000000000000000000001010101, 0b00000000000000000000000001010101, 0, 0));
    insn_series.push_back(synchronize_indie(
      2, 0, 0b00000000000000000000000001010101, 0b00000000000000000000000001010101, 0, 0));
    auto load_vcucode_0 = load_iteration_2<0>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR, 0);
    insn_series.push_back(load_vcucode_0);
    insn_series.push_back(load_iteration_2<0>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR + 32, 0));

    auto load_vcucode_2 = load_iteration_2<2>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR, 0);
    insn_series.push_back(load_vcucode_2);
    insn_series.push_back(load_iteration_2<2>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR + 32, 0));

    auto load_vcucode_4 = load_iteration_2<4>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR, 0);
    insn_series.push_back(load_vcucode_4);
    insn_series.push_back(load_iteration_2<4>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR + 32, 0));

    auto load_vcucode_6 = load_iteration_2<6>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR, 0);
    insn_series.push_back(load_vcucode_6);
    insn_series.push_back(load_iteration_2<6>(MHA_VCUCODE_ADDR, num_vcucodes - 1, 0, 0, 0, MASTER_VCUCODE_ADDR + 32, 0));
    ...
    insn_series.push_back(load_iteration_2<4>(EXP_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
    insn_series.push_back(load_iteration_2<4>(EXP_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
    insn_series.push_back(load_iteration_2<4>(SWISH_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 32, 0));
    insn_series.push_back(load_iteration_2<4>(SWISH_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 160, 0));
    insn_series.push_back(load_iteration_2<4>(REC_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 64, 0));
    insn_series.push_back(load_iteration_2<4>(REC_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 192, 0));
    insn_series.push_back(load_iteration_2<6>(EXP_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
    insn_series.push_back(load_iteration_2<6>(EXP_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR, 0));
    insn_series.push_back(load_iteration_2<6>(SWISH_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 32, 0));
    insn_series.push_back(load_iteration_2<6>(SWISH_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 160, 0));
    insn_series.push_back(load_iteration_2<6>(REC_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 64, 0));
    insn_series.push_back(load_iteration_2<6>(REC_LUT_ADDR, 64 * 128 / 256 - 1, 0, 0, 0, MASTER_VCULUT_ADDR + 192, 0));

    return {insn_series, opcode};
  }

  void set_vcucode(std::vector<uint64_t>& opcode, const Argument& args)
  {
    /** copy */
    ...
    /** Scale Product */
    auto vcucode = vcu::asm_vcu_op({"mulc psum, reg1, " + to_string_with_precision(std::sqrt(1.0f / args.d_h), 10)});
    ...
    /** llama mlp */
    vcucode = vcu::asm_vcu_op({
      "mul psum resadd, reg0",  // addr 10
      "fswish reg0, reg1"       // addr 11
    });
    ...
    vcucode = vcu::asm_vcu_op({
      /** Clean vcures */
      "config reg0, 0.0",  // addr 12

      /** Compute x^2 */
      "mul psum psum, reg0",  // addr 13

      /** Reduce sum to compute sum(x^2) */
      "redsum psum, reg0, 32",  // addr 14
      "add reg0 resadd, reg1",  // addr 15

      /** Mul by 1/n, rsqrt */
      "mulc resadd, reg0, " + to_string_with_precision(1.0f / args.d_model, 20),  // addr 16
      "rsqrt resadd, reg0",                                                       // addr 17

      /** Mul by x */
      "mul psum resadd, reg0",  // addr 18
    });
...
//           uint64_t weight_query_offset = (current_k * n_group_size * k_group_size + current_n * args.d_model * n_group_size) *
//           bytes_weight
...
//             (current_k * n_group_size * k_group_size + current_n * args.d_model * n_group_size) * bytes_weight +
//             args.weight_key_base_addr;
