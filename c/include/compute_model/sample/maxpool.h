      ...
      for (int w_iter = 0; w_iter < ofmap_w; ++w_iter) {
        for (int ic_iter = 0; ic_iter < ic_group; ++ic_iter) {
          for (int i = 0; i < ic_group_size; ++i) {
            max_value[i] = std::numeric_limits<float>::lowest();
          }
          ...
          for (int kh_iter = 0; kh_iter < weight_h; ++kh_iter) {
            for (int kw_iter = 0; kw_iter < weight_w; ++kw_iter) {
              if (h_iter * stride_h + kh_iter - pad_h >= 0 && w_iter * stride_w + kw_iter - pad_w >= 0
                ...
                if (DEBUG) {
                  std::cout << "ifmap_h_idx: " << ifmap_h_idx << " ifmap_w_idx: " << ifmap_w_idx << " ifmap_idx_start: " << ifmap_idx_start
                            << " ";
                }
                for (int i = 0; i < ic_group_size; ++i) {
                ...
                for (int i = 0; i < ic_group_size; ++i) {
                  max_value[i] = std::max((float)max_value[i], (float)args.input[ifmap_idx_start + i]);
                }
              }
              else {
                for (int i = 0; i < ic_group_size; ++i) {
                  max_value[i] = std::max((float)max_value[i], (float)(std::numeric_limits<float>::lowest()));
                }
              }
            }
          }
          for (int i = 0; i < ic_group_size; ++i) {
            args.output[ofmap_idx + i] = (TYPE_OUT)max_value[i];
          }
        }
      }
    }
