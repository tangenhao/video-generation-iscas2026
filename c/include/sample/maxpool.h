...
struct MaxpoolOp {
  ...
  std::vector<insn::instruction> operator()(Arguments args)
  {
    ...
    std::cout << args.block_channel_group << std::endl;
        ...
        for (int w_iter = 0; w_iter < w_iterations; ++w_iter) {
          ...
          for (int kh_iter = 0; kh_iter < kh_iterations; ++kh_iter) {
            for (int kw_iter = 0; kw_iter < kw_iterations; ++kw_iter) {
              ...
              // maxpool execute
              instruction_series.push_back(insn::maxpool(TYPE_IN == kFloat32  ? 2 :
                                                         TYPE_IN == kBfloat16 ? 1 :
                                                                                0,
                                                         TYPE_OUT == kFloat32  ? 2 :
                             ...
                             uint64_t psum_read_highaddr)
  {
    ...
    if (DEBUG) {
      ...
      std::cout << "sram_base_addr: " << (MASTER_PSUM_ADDR | (psum_read_highaddr << (8 + int(log2(oc_group_size * bytes_ofmap / 16)) - 1)))
                << std::endl;
    }
    ...
    return insn::load_iteration_3(ddr_base_addr,
                                  ...
                                  MASTER_PSUM_ADDR | (psum_read_highaddr << (8 + int(log2(oc_group_size * bytes_ofmap / 16)) - 1)),
                                  0);
  }
